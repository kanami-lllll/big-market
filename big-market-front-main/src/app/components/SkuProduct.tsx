...
import {activityStrategyArmory, creditPayExchangeSku, querySkuProductListByActivityId} from "@/apis";
import {getDemoActivityId, getDemoUserId} from "@/utils/demoParams";
...
export function SkuProduct({handleRefresh}) {
    ...
    const querySkuProductListByActivityIdHandle = async () => {
        const result = await querySkuProductListByActivityId(getDemoActivityId());
    ...
    const creditPayExchangeSkuHandle = async (sku: number) => {
        ...
        const armoryResult = await activityStrategyArmory(activityId);
        const {code: armoryCode, info: armoryInfo}: { code: string; info: string; data: boolean } = await armoryResult.json();

        if (armoryCode != "0000") {
            window.alert("装配活动库存，接口调用失败 code:" + armoryCode + " info:" + armoryInfo)
            return;
        }

        const result = await creditPayExchangeSku(getDemoUserId(), sku);
